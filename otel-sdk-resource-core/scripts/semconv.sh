...
declare -g packageDir="$(stack path --project-root)/otel-sdk-resource-core"
...
docker run --rm \
  -v "${tempDir}/opentelemetry-specification/semantic_conventions/resource:/source" \
  -v "${tempDir}/templates:/templates" \
  -v "${packageDir}/library/OTel/SDK/Resource/Core/:/output" \
  otel/semconvgen:${generatorVersion} \
  -f /source code \
  --template /templates/SemConvAttrs.hs.j2 \
  --output /output/Attributes.hs \
  -DschemaUrl=${schemaURL} \
  -Dmodule=OTel.SDK.Resource.Core.Attributes
